...
import fetchActiveUserChoices from './fetchActiveUserChoices';
jest.mock('@/shared/api/api');

const mockedApiRequest = jest.mocked(apiRequest);

describe('fetchActiveUserChoices', () => {
    const url = '/myActiveEventChoices';
    const method = 'GET';
    test('должен успешно получить событие', async () => {
        ...
        const thunk = new TestAsyncThunk<FetchUserChoicesResponse, FetchUserChoicesParams, string>(
            fetchActiveUserChoices,
        );
    ...
    test('должен вернуть ошибку при неуспешном ответе', async () => {
        ...
        expect(result.payload).toBe('fetchActiveUserChoices error');
    });
